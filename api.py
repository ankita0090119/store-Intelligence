...
for cam in [1, 2, 3, 4, 5]:
    fname = data_path("summaries", summary_filename(cam))
    ...
    for zone, count in cam_data.get("zone_popularity", {}).items():
        DATA["zone_popularity"][zone] = (
            DATA["zone_popularity"].get(zone, 0) + count
        )
...
print(
    f"[INFO] Loaded — footfall={DATA['footfall']}, "
    f"zones={list(DATA['zone_popularity'].keys())}, "
    f"anomalies={len(DATA['anomalies'])}"
)


# ── Load events from all cameras ──────────────────────

EVENTS = []

for cam in [1, 2, 3, 4, 5]:
    fname = data_path("events", f"events_cam{cam}.json")
...
def home():
    return {
        ...
        "cameras_loaded": sum(
            ...
            for cam in [1, 2, 3, 4, 5]
            if os.path.exists(
                data_path("summaries", summary_filename(cam))
            )
        ),
        "endpoints": [
            "/footfall",
...
def get_dwell():
    ...
    return {
        ...
        "dwell_times": [
            {"zone": zone, "avg_seconds": seconds}
            for zone, seconds in sorted_dwell
        ],
    }
...
def get_peak():
    return {
        ...
        "message": (
            f"Busiest at {DATA['peak_time_sec']}s "
            f"with {DATA['peak_count']} people"
        ),
    }
